...
(
   ...
   source_id            int not null comment '来源ID',
   source_type          char(1) not null comment '来源类型（）',
   primary key (id)
);
...
create VIEW  v_banner_images
    ...
    select v_game_activity.id, type, size1_url url, banner_desc from v_game_activity,db_images where v_game_activity.image = db_images.id ORDER BY focus_count desc, join_count desc LIMIT 0, 6;
...
create VIEW  v_game_activity
    ...
    SELECT
      ...
      (select count(1) from op_join_game where op_join_game.game_id=db_game.id) join_count
    FROM
      db_game
    UNION ALL
    SELECT
      ...
      (select count(1) from op_join_activity where op_join_activity.activity_id=db_activity.id) join_count
    FROM    
      db_activity;
