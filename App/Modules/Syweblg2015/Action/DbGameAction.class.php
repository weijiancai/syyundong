...
class DbGameAction extends CommonAction
{
    ...
    */
    ...
    public function score()
    {
        $model = D('OpGameScore');
        $id = $_REQUEST['game_id'];
        $map = $this->score_search('', $id);
        $this->assign('map', $map);
        if (method_exists($this, '_filter')) {
            $this->_filter($map);
        }
        if (!empty ($model)) {
            $this->score_list($model, $map);
        }
        //查询赛事分组
        $this->assign('game_group',M('OpGameGroup')->where('game_id='.$id)->select());
        $this->display();
    }

    protected function score_search($name = '', $id)
    {
        $model = D('OpGameScore');
        $map = array();
    ...
    protected function score_list($model, $map, $sortBy = '', $asc = true)
    {
        $pk = $model->getPk();
